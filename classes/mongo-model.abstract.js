...
class MongoModel extends orm_1.ConnectableModel {
    ...
    static disconnect(connectionName = this.getConnectionName()) {
    ...
    saveEmitter(options) {
        return this.constructor.setCollection()
            .then(() => {
            return this.constructor.collection.insertOne(this.formalize(), options)
                .then((value) => {
                return value.ops[0];
            });
        });
    }
    updateEmitter(options = { upsert: true }) {
        return this.constructor.setCollection()
            .then(() => {
